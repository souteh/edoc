...
public class TypeDeContenuResourceIntTest {
     ...
     *
     * @see ma.edoc.cr.repository.search.TypeDeContenuSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createTypeDeContenu() throws Exception {
        ...
        // Validate the TypeDeContenu in Elasticsearch
        verify(mockTypeDeContenuSearchRepository, times(1)).save(testTypeDeContenu);
    }
    ...
    public void createTypeDeContenuWithExistingId() throws Exception {
        ...
        // Validate the TypeDeContenu in Elasticsearch
        verify(mockTypeDeContenuSearchRepository, times(0)).save(typeDeContenu);
    }
    ...
    public void deleteTypeDeContenu() throws Exception {
        ...
        // Validate the TypeDeContenu in Elasticsearch
        verify(mockTypeDeContenuSearchRepository, times(1)).deleteById(typeDeContenu.getId());
    }

    @Test
    @Transactional
    public void searchTypeDeContenu() throws Exception {
        // Initialize the database
        typeDeContenuRepository.saveAndFlush(typeDeContenu);
        when(mockTypeDeContenuSearchRepository.search(queryStringQuery("id:" + typeDeContenu.getId())))
            .thenReturn(Collections.singletonList(typeDeContenu));
        // Search the typeDeContenu
        restTypeDeContenuMockMvc.perform(get("/api/_search/type-de-contenus?query=id:" + typeDeContenu.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(typeDeContenu.getId().intValue())))
            .andExpect(jsonPath("$.[*].denominationAr").value(hasItem(DEFAULT_DENOMINATION_AR.toString())))
