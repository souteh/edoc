package ma.edoc.cr.web.rest;

import ma.edoc.cr.EdocApp;
...
import ma.edoc.cr.web.rest.errors.ExceptionTranslator;
...
import static ma.edoc.cr.web.rest.TestUtil.createFormattingConversionService;
...
@SpringBootTest(classes = EdocApp.class)
public class FondDocumentResourceIntTest {
    ...
    /**
     * This repository is mocked in the ma.edoc.cr.repository.search test package.
     *
     * @see ma.edoc.cr.repository.search.FondDocumentSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createFondDocument() throws Exception {
        ...
        // Validate the FondDocument in Elasticsearch
        verify(mockFondDocumentSearchRepository, times(1)).save(testFondDocument);
    }
    ...
    public void createFondDocumentWithExistingId() throws Exception {
        ...
        // Validate the FondDocument in Elasticsearch
        verify(mockFondDocumentSearchRepository, times(0)).save(fondDocument);
    }
    ...
    public void deleteFondDocument() throws Exception {
        ...
        // Validate the FondDocument in Elasticsearch
        verify(mockFondDocumentSearchRepository, times(1)).deleteById(fondDocument.getId());
    }

    @Test
    @Transactional
    public void searchFondDocument() throws Exception {
        // Initialize the database
        fondDocumentRepository.saveAndFlush(fondDocument);
        when(mockFondDocumentSearchRepository.search(queryStringQuery("id:" + fondDocument.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(fondDocument), PageRequest.of(0, 1), 1));
        // Search the fondDocument
        restFondDocumentMockMvc.perform(get("/api/_search/fond-documents?query=id:" + fondDocument.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(fondDocument.getId().intValue())))
            .andExpect(jsonPath("$.[*].denominationAr").value(hasItem(DEFAULT_DENOMINATION_AR.toString())))
